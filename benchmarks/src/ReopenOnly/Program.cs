...
const string ExperimentId = "reopen-only";
const string ExperimentTitle = "Reopen existing prepared storage only.";
...
{
    Directory.CreateDirectory(dir);
    // TODO setup built storage first, then measure only open/refresh/index readiness; first lookup should be validation or separate metric.

...
const string ExperimentId = "append-only";
const string ExperimentTitle = "Append rows to an existing storage only.";
...
{
    Directory.CreateDirectory(dir);
    // TODO setup existing storage first, then measure only AppendElement batch and required flush; do not rebuild in append-only.

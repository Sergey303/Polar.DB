...
const string ExperimentId = "build-only";
const string ExperimentTitle = "Build/index preparation only. Data load is setup.";
...
{
    Directory.CreateDirectory(dir);
    // TODO setup loaded storage first, then measure only Polar.DB Build()/index persistence; SQLite should measure CREATE INDEX only.

...
child processes, waits for both workers, validates compatible result shapes, and then creates
the combined report.

Generated latest-result artifacts:

- `<experiment>.html` — derived report;
- `<experiment>.manifest.json` — environment, Git commit and worker process metadata;
...
use the explicit sample profiles from `BenchmarkDefaults`.

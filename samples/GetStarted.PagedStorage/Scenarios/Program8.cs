...
namespace GetStarted.PagedStorage.Scenarios
{
    internal sealed class Program8AdvancedPagedKeyValueScenario : ISampleScenario
    {
        public string Id => "gs1-p8";
        public string Title => "Advanced paged key-value storage with portions and range helpers";
        public string SourcePath => "samples/GetStarted1/Program8.cs";
        public bool IsExtractedFragment => false;
        private static string dbpath = SamplePaths.Root;
        ...
        public static void Main8_1()
        {
            ...
            foreach (int key in arr)
            {
                ...
                int ind = Program8AdvancedPagedKeyValueScenario.Get8FirstIndexOf(start, numb, arr, key);
                if (ind == -1)
                {
                    Console.WriteLine($"no ind for key {key}");
                    int ind2 = Program8AdvancedPagedKeyValueScenario.Get8FirstIndexOf(start, numb, arr, key);
                }
                else
                {
                    int keyval = arr[ind];
    ...
    class KVSequencePortion
    {
        ...
        public void Activate()
        {
            ...
            GetDia = Program8AdvancedPagedKeyValueScenario.GetDiaFunc3(keys_arr);
        }
        ...
        public object Get(int search_key)
        {
            ...
            Diapason diap = GetDia(search_key);
            //int ifirst = 666888;
            int ifirst = Program8AdvancedPagedKeyValueScenario.Get8FirstIndexOf((int)diap.start, (int)diap.numb, keys_arr, search_key);
            //int ifirst = Program8AdvancedPagedKeyValueScenario.Get8FirstIndexOf(0, keys_arr.Length, keys_arr, search_key);
            if (ifirst == -1)
            {
                return null;
            }
